...
Try{
Get-Service -Name $ServiceName -ComputerName $server  | Stop-Service
LogWrite "$ServiceName Stopped on $server"
}
Catch{
LogWrite "$ServiceName Failed to Stop on $server"
}
Try{
Get-Service -Name $ServiceName -ComputerName $server  | Set-Service -StartupType Disabled
LogWrite "$ServiceName Disabled on $server"
}
Catch{
LogWrite "$ServiceName Failed to Disable Service on $server"
}
}   
Stop-Transcript